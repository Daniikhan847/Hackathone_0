...
const DetailsSection = () => {
  return (
    <section className="flex flex-col md:w-1/2 w-full text-text2 sm:gap-5 gap-3 mmd:px-14 px-5 py-5">
      <h1 className="sm:text-4xl text-2xl text-blue-950 font-clash">
        The Dandy Chair
      </h1>
      <h3 className="sm:text-xl text-lg text-black">£250</h3>
      <span className="sm:mt-4">
        <h3 className="text-blue-950 font-clash">Description</h3>
...
const Dimensions = () => {
  return (
    <span className="flex flex-col gap-2 mt-5 max-sm:text-sm">
      <h3 className="text-blue-950 capitalize font-clash text-nr">
        dimensions
      </h3>
            ...
            <span key={ind} className="h-full w-full flex items-center">
              <h3
                className={`capitalize ${
                  ind > 2 ? "text-text2" : "text-blue-950 font-clash"
                }`}
              >
                {val}
              </h3>
  ...
  return (
    <div className="w-full flex justify-between">
      <span className="flex items-center gap-4">
        <h3 className="capitalize text-blue-950 font-clash">amount:</h3>
        <span className="flex sm:w-28 w-20 sm:h-12 h-8 bg-gray-300 items-center justify-around">
          {["-", "1", "+"].map((val, ind) => (
            <span
              key={ind}
              className={`${
                val === "1" ? "text-blue-950" : "text-text2/50"
              }`}
            >
              {val}
            </span>
